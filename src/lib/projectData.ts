...
export const stacks: ProjectsStack[] = [
  {
    name: "D-order",
    description: "축제 부스 운영지원 웹 서비스 (Customer/Admin/Server)",
    overview:
      "D-order는 축제 부스 현장에서 발생하는 주문·결제·서빙·정산의 불편함을 디지털로 해결하는 운영지원 서비스입니다.\nCustomer(고객), Admin(운영진), Server(서빙) 3개의 웹을 분리해 각 역할에 필요한 흐름만 제공하고,\n현장 네트워크 환경을 고려해 주문/상태/직원 호출을 실시간으로 동기화했습니다.",
    detailDescription:
      "축제 부스 운영의 핵심은 '실시간'과 '역할 분리'라고 판단했습니다.\nCustomer는 주문과 결제 흐름에 집중하고, Admin은 테이블/주문/매출 현황을 관리하며, Server는 서빙 요청을 처리합니다.\nWebSocket 기반 이벤트(주문 상태 변경·테이블 초기화·직원 호출)를 설계하고,\nRedis Pub/Sub로 Front↔Back 간 이벤트를 연결해 현장에서도 끊김 없는 운영 경험을 만들었습니다.",
    categories: ["React", "TypeScript", "Team"],
    feature: true,
    image: "/images/projectsImage/D-order.webp",
    SemiInfo: [
      "1차 2025.03 ~ 2025.05\n2차 2025.05 ~ 2025.07\n3차 2025.07 ~ 2025.09\n4차 2026.02 ~ (3차 리팩토링/고도화)",
      "Team project (FE 3 / BE 4 / PM 1 / Designer 1)",
      "Frontend Lead",
      "https://github.com/D-Order/2026-d-order-fe-admin-v3 \n https://github.com/D-Order/2026-d-order-fe-customer-v3 \n https://github.com/D-Order/2026-d-order-fe-server-v3 \n https://github.com/D-Order/2026-d-order-backend-v3",
      "http://customer.dorder-api.shop \n http://admin.dorder-api.shop \n http://server.dorder-api.shop",
    ],
    stack: [
      "React + TypeScript - 역할(고객/운영진/서빙)별 UI를 분리하면서도, 실시간 상태 변화가 많은 운영 도메인을 타입 안정적으로 다루기 위해 사용했습니다.",
      "WebSocket - 주문/테이블/직원 호출의 상태를 실시간으로 동기화하기 위한 핵심 채널로 사용했습니다.",
      "Redis Pub/Sub - Front에서 발생한 이벤트가 Back에서 처리된 뒤 다시 다른 클라이언트로 퍼지도록, 실시간 이벤트 브로커로 구성했습니다.",
      "JWT/쿠키 인증 + 권한 분리 - 고객/직원/관리자 권한 범위를 분리해 각 역할이 필요한 이벤트만 구독하도록 채널을 설계했습니다.",
      "연결 안정화(Heartbeat/Reconnect) - 현장 네트워크 변동을 고려해 PING/PONG(30s), 재연결(3s, 최대 5회)로 실시간 UX 붕괴를 방지했습니다.",
    ],
    features: {
      problem: [
        "축제 부스 주문을 수기로 처리하면서 주문 누락/대기 증가/정산 혼선이 발생",
        "고객은 메뉴 확인·입금·확인 과정이 분산되어 흐름이 끊김",
        "운영진은 테이블별 주문/상태를 실시간으로 파악하기 어려움",
      ],
      user: [
        "QR 기반 테이블 입장",
        "메뉴 조회/장바구니/주문 생성",
        "계좌 복사 기반 결제 안내",
        "송금 확인 요청 → 실시간 처리 상태 안내(WebSocket)",
      ],
      admin: [
        "실시간 주문/테이블 현황 대시보드",
        "조리 상태 변경(조리중→조리완료→서빙중→서빙완료) 전이 관리",
        "테이블 초기화/병합 등 운영 기능",
        "매출/정산 등 운영 지표 확인",
      ],
    },
    roles: [
      ...
      "React 기반 장바구니·주문·결제 흐름과 화면 간 상태 관리 아키텍처 설계",
      "축제 현장 네트워크를 고려한 인증/연결 전략 설계 및 장애 대응 UX 정리",
      "권한 범위 분리를 위한 고객·직원 WebSocket 채널 설계 (구독 범위 분리)",
      "실시간 상태 동기화 구조 구현: WebSocket + Redis Pub/Sub 기반 이벤트 전파",
      "2주 내 MVP 협업을 위한 개발 기준/문서화(와이어프레임 정리, API 명세 공유, GitHub→Discord 알림 자동화 등) 주도",
    ],
    challenges: [
      {
        problem:
          "축제 현장 네트워크를 재현하기 위해 모바일 환경의 CPU slowdown 조건에서 성능을 측정했을 때,\nradial-gradient + blur 조합 배경이 GPU 과부하를 유발하며 프레임 저하와 렌더링 지연이 발생했습니다.\n또한 특정 기기에서 배경이 깨지거나(렌더 아티팩트) 콘텐츠 표시가 늦어지는 현상이 있었습니다.",
        solution:
          "현상을 '현장 조건'에서 재현하는 것이 우선이라 판단해, Chrome DevTools + Lighthouse로 모바일 조건(CPU slowdown)을 적용해 문제를 의도적으로 재현했습니다.\n이후 아래 순서로 원인을 좁혔습니다.\n- Performance 기록으로 입력 이벤트 구간의 Main Thread 작업과 프레임 드랍 지점을 확인\n- Paint Flashing으로 blur 배경이 스크롤/입력마다 repaint를 유발하는지 검증\n- Layers 패널로 blur 효과가 별도 합성 레이어로 분리되지 못하고 메인 스레드에 부담을 주는지 확인\n원인을 blur 렌더링/합성 비용으로 확정한 뒤, 배경을 이미지화하고 GPU-friendly한 transform 중심 구성으로 바꿔 composite 단계에서 처리되도록 전환했습니다.\n결과적으로 모바일 환경에서 체감 렌더 지연과 화면 깨짐 현상이 사라졌습니다.",
        learnings:
          "모바일 성능 이슈는 '느리다'가 아니라 '어떤 조건에서 깨지는가'를 재현하는 것부터가 문제 해결의 시작임을 배웠고,\n감각적인 UI도 DevTools 기반 근거를 바탕으로 안전한 렌더링 방식으로 설계해야 한다는 기준을 얻었습니다.",
      },
      {
        problem:
          "장바구니 상태를 클라이언트 전역 상태(예: Zustand) 중심으로 관리했을 때,\n새로고침/로그인 상태 변경/다중 탭 등에서 상태 유실이 발생했고 현장 운영 시 신뢰성이 떨어졌습니다.",
        solution:
          "원인을 '클라이언트 메모리 기반 상태'의 구조적 한계로 보고, 해결을 위해 여러 접근을 비교/실험했습니다.\n- (시도) localStorage에 장바구니를 저장해 새로고침 복구 → 로그아웃/테이블 변경 시점에 상태 정리가 어려웠고, 오래된 데이터가 남는 문제가 발생\n- (시도) BroadcastChannel로 탭 간 상태 동기화 → 탭 간 전파는 되지만 결국 상태의 기준이 메모리라 예외 케이스(로그인/세션 변경)에서 정합성이 깨짐\n- (시도) React Query cache를 단일 캐시로 쓰는 방식 검토 → 캐시는 '정합성의 단일 출처'가 아니라서, 재입장/세션 전환에 대한 규칙을 안정적으로 만들기 어려움\n결론적으로 서버를 단일 출처로 두는 구조로 전환했습니다.\n- 서버가 장바구니/주문 스냅샷을 관리\n- 클라이언트는 화면에서 필요한 시점에 스냅샷을 조회/구독(WebSocket)해 동기화\n이 구조로 새로고침/다중 탭/세션 변경에서도 상태가 서버 기준으로 수렴하도록 만들었습니다.",
        learnings:
          "상태 관리는 '편의성'보다 '신뢰성'이 우선이며, 특히 현장 운영 시스템에서는 서버를 기준으로 정합성을 맞추는 설계가 필요하다는 점을 체감했습니다.",
      },
      {
        problem:
          "직원 호출 기능에서 여러 직원이 동시에 '수락'을 누를 때, 동일 요청이 중복 처리되거나 화면 상태가 꼬이는 동시성 문제가 발생할 수 있었습니다.",
        solution:
          "처음에는 '요청 수락'을 단순 API 호출로 처리했는데, 현장에서는 동시에 여러 명이 눌렀을 때 중복 처리될 여지가 컸습니다.\n그래서 동시성 자체를 설계 대상으로 보고 아래처럼 개선했습니다.\n- 직원 호출은 WebSocket으로 상태(PENDING/ACCEPTED/DELETED)를 구독하게 하고, 클라이언트는 이 상태를 기준으로 UI를 전환\n- 서버에서는 수락 처리에 트랜잭션/락(비관적 락 기반)을 적용해 PENDING → ACCEPTED 전이가 단 한 번만 발생하도록 보장\n- 처리 결과를 Redis Pub/Sub로 브로드캐스트해서, Admin/Server/Customer가 동일한 이벤트를 받아 같은 상태로 수렴\n이후에는 어떤 클라이언트가 먼저 눌렀는지와 상관없이, 최종 상태가 단일하게 결정되고 화면도 동일하게 동기화되었습니다.",
        learnings:
          "실시간 도메인의 핵심 난점은 동시성/정합성이고, 프론트는 UX만이 아니라 '상태가 수렴하는 구조'를 함께 설계해야 한다는 걸 배웠습니다.",
      },
    ],
  },
  {
