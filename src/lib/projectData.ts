...
export const stacks: ProjectsStack[] = [
  {
    ...
    challenges: [
      ...
      },
      {
        ...
        solution:
          "원인을 '클라이언트 메모리 기반 상태'의 구조적 한계로 보고, 해결을 위해 여러 접근을 비교/실험했습니다.\n- localStorage에 장바구니를 저장해 새로고침 복구 → 로그아웃/테이블 변경 시점에 상태 정리가 어려웠고, 오래된 데이터가 남는 문제가 발생\n- BroadcastChannel로 탭 간 상태 동기화 → 탭 간 전파는 되지만 결국 상태의 기준이 메모리라 예외 케이스(로그인/세션 변경)에서 정합성이 깨짐\n- React Query cache를 단일 캐시로 쓰는 방식 검토 → 캐시는 '정합성의 단일 출처'가 아니라서, 재입장/세션 전환에 대한 규칙을 안정적으로 만들기 어려움\n결론적으로 서버를 단일 출처로 두는 구조로 전환했습니다.\n- 서버가 장바구니/주문 스냅샷을 관리\n- 클라이언트는 화면에서 필요한 시점에 스냅샷을 조회/구독(WebSocket)해 동기화\n이 구조로 새로고침/다중 탭/세션 변경에서도 상태가 서버 기준으로 수렴하도록 만들었습니다.",
        learnings:
